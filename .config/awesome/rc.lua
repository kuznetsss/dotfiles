...
memwidget = wibox.container.constraint(memwidget, 'max', 50, 10)
memwidget.widget:set_vertical(true)
memwidget.widget:set_background_color("#494B4F")
memwidget.widget:set_border_color(nil)
memwidget.widget:set_color({ type = "linear", from = { 0, 0 }, to = { 10,0 }, stops = { {0, "#40FF7A"} }})
-- Register widget
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey, }, "z", function () awful.spawn.with_shell("xzoom") end,
	{description="Open screen magnifier", group="client"}),
...
function run_once(prg,arg_string,pname)
	...
	if not arg_string then
		awful.spawn.with_shell("(pgrep -u $USER " .. pname .. ") || (" .. prg .. ")")
	else
		awful.spawn.with_shell("(pgrep -u $USER " .. pname .. ") || (" .. prg .. " " .. arg_string .. ")")
		end
end
